...
When the user explicitly asks for a docs subagent, docs agent, or delegated docs work, use the project-scoped `uitweaks_docs` custom agent from `.codex/agents/uitweaks-docs.toml`. Tell the subagent to read [this docs reference](6.docs.md) before acting.
