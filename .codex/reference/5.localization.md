...
All 19 language files must have the same key set. Add, remove, or rename keys across every file in one change. For translation wording corrections, edit only the affected languages and leave unrelated translations untouched.
...
2. Decide whether the change affects the shared key structure or only wording in specific languages.
3. For each affected language, think through what the string needs to communicate in that language and context. Consider multiple possible phrasings when useful, compare clarity, length, tone, and game-UI naturalness, then choose the best option.
4. For key additions, removals, or renames, update all 19 files in one pass. For wording corrections, update only the affected language files. Preserve flat JSON objects and existing two-space indentation.
5. Verify that every language file has the same key set.
6. Verify that no changed non-English value uses English fallback text.
