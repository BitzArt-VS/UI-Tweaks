...
Enable Zoom in the in-game config dialog, then hold `Z` to zoom in. The effect eases in and out and can add a subtle vignette around the screen.
...
- **Meal remaining indicator** — partially filled meal bowls show a vanilla-style indicator in item slots.
