...
internal sealed class GuiInputRouter
{
    ...
    private readonly GuiCallbackDispatcher _callbackDispatcher;
        ...
        Func<IGuiNode?> getRootNode)
    {
        ...
        _callbackDispatcher = new GuiCallbackDispatcher(clientApi.Logger);
    ...
    {
        if (region.Target.SupportedResizeEdges == GuiEdge.None)
        {
            return;
        }
    ...
    private bool DispatchMouseWheel(int physicalX, int physicalY, float delta)
    {
        ...
        for (int i = _interactiveRegions.Count - 1; i >= 0; i--)
        {
            ...
            if (region.OnMouseWheel is null)
            {
                continue;
            }
            ...
            _callbackDispatcher.Dispatch(
                region.OnMouseWheel,
                MakeMouseArgs(physicalX, physicalY, logicalX, logicalY, EnumMouseButton.None) with { WheelDelta = delta });
            return true;
        }
        return false;
    }
    ...
    private bool DispatchMouseDown(MouseEvent args)
    {
        ...
        var focusTarget = region.ShouldRequestFocus(mouseArgs)
            ? region.MouseDownFocusTarget
            : null;

        SetFocusedNode(focusTarget);
        _callbackDispatcher.Dispatch(region.OnMouseDown, mouseArgs);
    ...
    private bool DispatchMouseUp(MouseEvent args)
    {
        ...
        _callbackDispatcher.Dispatch(onMouseUp, mouseArgs);
        if (insideCapture)
        {
            _callbackDispatcher.Dispatch(onMouseClick, mouseArgs);
        }
    ...
    {
        if (_capturedOnMouseMove is null)
        {
            return true;
        }
        ...
        _convertToLogical(args.X, args.Y, out double logicalX, out double logicalY);
        var mouseArgs = MakeMouseArgs(args.X, args.Y, logicalX, logicalY, args.Button);
        _callbackDispatcher.Dispatch(_capturedOnMouseMove, mouseArgs);
        return true;
    }
    ...
    private void LeaveHoveredRegion(GuiMouseEventArgs mouseArgs)
    {
        ...
        _callbackDispatcher.Dispatch(_hoveredOnMouseLeave, mouseArgs);
        _hoveredToken = null;
    ...
    private void EnterHoveredRegion(object? newToken, GuiMouseEventArgs mouseArgs)
    {
        ...
        for (int i = _interactiveRegions.Count - 1; i >= 0; i--)
        {
            ...
            _callbackDispatcher.Dispatch(_interactiveRegions[i].OnMouseEnter, mouseArgs);
            break;
        }
    }
    ...
    private void DispatchMouseMoveToHoveredRegion(object? token, GuiMouseEventArgs mouseArgs)
    {
        ...
        for (int i = _interactiveRegions.Count - 1; i >= 0; i--)
        {
            ...
            _callbackDispatcher.Dispatch(_interactiveRegions[i].OnMouseMove, mouseArgs);
            return;
        }
    }
    ...
    private bool DispatchKeyToToken(object token, GuiKeyEventKind kind, KeyEvent args)
    {
        ...
        for (int i = 0; i < _keyboardRegions.Count; i++)
        {
            ...
            _keyboardRegions[i].Dispatch(kind, keyArgs, _callbackDispatcher);
            return true;
        }
    ...
    private bool DispatchFocusChanged(IGuiNode? node, bool focused)
    {
        ...
        for (int i = 0; i < _keyboardRegions.Count; i++)
        {
            ...
            _callbackDispatcher.Dispatch(_keyboardRegions[i].OnFocusChanged, focused);
            return true;
        }
        return false;
    }
