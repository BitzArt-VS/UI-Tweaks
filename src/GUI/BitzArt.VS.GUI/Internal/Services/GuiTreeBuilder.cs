...
internal sealed class GuiTreeBuilder : IGuiTreeBuilder, IDisposable
{
    ...
    private GuiBounds? PaintInto(Context context, bool registerRegions, GuiBounds? inheritedClipBounds)
    {
        ...
        foreach (GuiSlot slot in _renderOrder)
        {
            ...
            var componentBounds = componentSlot.Bounds;
            if (componentBounds is null)
            {
                continue;
            }

            var bounds = componentBounds.Value.Bounds;
    ...
    {
        if (slot.Instance is IGuiResizable resizable && resizable.SupportedResizeEdges != GuiEdge.None)
        {
        ...
        if (slot.HasMouseHandlers)
        {
            _renderer.AddInteractiveRegion(new InteractiveRegion(
                ...
                slot.OnMouseLeave,
                slot.OnMouseWheel,
                slot.MouseDownFocusTarget,
                slot.MouseDownFocusCondition,
    ...
    private struct SlotCallbacks
    {
        ...
        public IGuiNode? MouseDownFocusTarget;
        public Predicate<GuiMouseEventArgs>? MouseDownFocusCondition;
        ...
        public void AddMouseHandler(GuiMouseEventKind kind, GuiCallback<GuiMouseEventArgs> callback)
        {
            switch (kind)
            {
                case GuiMouseEventKind.Down:
                    OnMouseDown = Combine(OnMouseDown, callback);
                    break;
                case GuiMouseEventKind.Up:
                    OnMouseUp = Combine(OnMouseUp, callback);
                    break;
                case GuiMouseEventKind.Click:
                    OnMouseClick = Combine(OnMouseClick, callback);
                    break;
                case GuiMouseEventKind.Move:
                    OnMouseMove = Combine(OnMouseMove, callback);
                    break;
                case GuiMouseEventKind.Enter:
                    OnMouseEnter = Combine(OnMouseEnter, callback);
                    break;
                case GuiMouseEventKind.Leave:
                    OnMouseLeave = Combine(OnMouseLeave, callback);
                    break;
                case GuiMouseEventKind.Wheel:
                    OnMouseWheel = Combine(OnMouseWheel, callback);
                    break;
            }
        }
        ...
        public void AddKeyHandler(GuiKeyEventKind kind, GuiCallback<GuiKeyEventArgs> callback)
        {
            switch (kind)
            {
                case GuiKeyEventKind.Down:
                    OnKeyDown = Combine(OnKeyDown, callback);
                    break;
                case GuiKeyEventKind.Up:
                    OnKeyUp = Combine(OnKeyUp, callback);
                    break;
                case GuiKeyEventKind.Press:
                    OnKeyPress = Combine(OnKeyPress, callback);
                    break;
            }
        }
        ...
        {
            OnFocusChanged = Combine(OnFocusChanged, callback);
        }

        public void SetMouseDownFocusTarget(
            IGuiNode target,
            Predicate<GuiMouseEventArgs>? condition)
        {
            MouseDownFocusTarget = target;
            MouseDownFocusCondition = condition;
        }
        ...
        public static SlotCallbacks Combine(SlotCallbacks ownCallbacks, SlotCallbacks externalCallbacks)
        {
            ...
            {
                OnMouseDown = Combine(ownCallbacks.OnMouseDown, externalCallbacks.OnMouseDown),
                OnMouseUp = Combine(ownCallbacks.OnMouseUp, externalCallbacks.OnMouseUp),
                OnMouseClick = Combine(ownCallbacks.OnMouseClick, externalCallbacks.OnMouseClick),
                OnMouseMove = Combine(ownCallbacks.OnMouseMove, externalCallbacks.OnMouseMove),
                OnMouseEnter = Combine(ownCallbacks.OnMouseEnter, externalCallbacks.OnMouseEnter),
                OnMouseLeave = Combine(ownCallbacks.OnMouseLeave, externalCallbacks.OnMouseLeave),
                OnMouseWheel = Combine(ownCallbacks.OnMouseWheel, externalCallbacks.OnMouseWheel),
                OnKeyDown = Combine(ownCallbacks.OnKeyDown, externalCallbacks.OnKeyDown),
                OnKeyUp = Combine(ownCallbacks.OnKeyUp, externalCallbacks.OnKeyUp),
                OnKeyPress = Combine(ownCallbacks.OnKeyPress, externalCallbacks.OnKeyPress),
                OnFocusChanged = Combine(ownCallbacks.OnFocusChanged, externalCallbacks.OnFocusChanged),
                MouseDownFocusTarget =
                    externalCallbacks.MouseDownFocusTarget
                    ?? ownCallbacks.MouseDownFocusTarget,
                MouseDownFocusCondition =
                    externalCallbacks.MouseDownFocusTarget is not null
                        ? externalCallbacks.MouseDownFocusCondition
                        : ownCallbacks.MouseDownFocusCondition,
            };
        }

        private static GuiCallback<T>? Combine<T>(GuiCallback<T>? first, GuiCallback<T>? second)
        {
            if (first is null)
            {
                return second;
            ...
            return second is null ? first : first.Value + second.Value;
        }

        public readonly void ApplyTo(GuiSlot slot)
        {
            ...
            slot.MouseDownFocusTarget = MouseDownFocusTarget;
            slot.MouseDownFocusCondition = MouseDownFocusCondition;
        }
    }
        ...
        where T : IGuiNode, new()
    {
        ...
        IGuiSlotBuilder IGuiSlotBuilder.SetMouseDownFocusTarget(
            IGuiNode target,
            Predicate<GuiMouseEventArgs>? condition)
        {
            _externalCallbacks.SetMouseDownFocusTarget(target, condition);
            return this;
        }
        ...
        private sealed class SlotBuilder(TreeFrame<T> frame, IGuiNode instance) : IGuiSlotBuilder
        {
            ...
            IGuiSlotBuilder IGuiSlotBuilder.SetMouseDownFocusTarget(
                ...
                Predicate<GuiMouseEventArgs>? condition)
            {
                frame._ownCallbacks.SetMouseDownFocusTarget(target, condition);
                return this;
            }
        }
    }
