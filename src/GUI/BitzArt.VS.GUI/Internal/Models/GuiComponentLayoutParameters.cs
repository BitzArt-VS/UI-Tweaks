...
public sealed class GuiComponentLayoutParameters
{
    ...
    public GuiComponentBounds ResolveProvisionalBounds(
        GuiBounds availableBounds,
        GuiComponentBounds? previousBounds)
        => previousBounds is null
            ? ResolveBounds(availableBounds)
            : ResolveBounds(
                availableBounds,
                previousBounds.Value.Bounds.Size,
    ...
    private GuiComponentBounds ResolveBounds(
        GuiBounds availableBounds,
        GuiSize? candidateSize,
        bool useAvailableSize)
    {
        GuiSize? availableSize =
            availableBounds.Size - Margin;
        ...
        var bounds = new GuiBounds(
            ResolvePosition(availableSize, resolvedSize),
            resolvedSize);

        return new GuiComponentBounds(bounds, Margin, Padding);
    }
        ...
        GuiSize resolvedSize)
    {
        ...
        var position =
            alignedPosition.Value.Offset(Margin.Left, Margin.Top);

        return Position is null
            ? position
            : position.Offset(Position.Value.X, Position.Value.Y);
    }
