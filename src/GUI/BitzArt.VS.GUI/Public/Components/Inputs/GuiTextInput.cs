...
/// edits via <see cref="SetTextAsync"/> are also enforced.
/// </para>
/// <para>
...
public sealed class GuiTextInput : GuiInputBase
{
    ...
    /// prefer <see cref="SetTextAsync"/> for an external write that should still respect
    ...
    protected override void ConfigureSlot(IGuiSlotBuilder builder)
    {
        ...
        builder.OnKeyDown((System.Func<GuiKeyEventArgs, ValueTask>)HandleKeyDownAsync);
        builder.OnKeyPress((System.Func<GuiKeyEventArgs, ValueTask>)HandleKeyPressAsync);
        builder.OnFocusChanged((Action<bool>)HandleFocusChanged);
    ...
    public async ValueTask<bool> SetTextAsync(string text)
    {
        ...
        if (OnTextChanged is GuiCallback<string> onTextChanged)
        {
            ...
            && _spinnerDownTarget is not null)
        {
            ...
            var parentPosition =
                ...
                ?? throw new InvalidOperationException(
                    "Text-input spinner targets require a resolved parent position.");

            var position =
                provisionalBounds.Bounds.Position?.Resolve(parentPosition)
                ?? throw new InvalidOperationException(
                    "Text-input spinner targets require a resolved position.");

            var size =
                provisionalBounds.Bounds.Size
                ?? throw new InvalidOperationException(
    ...
    protected override void BuildComponentTree(IGuiTreeBuilder builder)
    {
        ...
        builder.Add<GuiMouseTarget>(1)
            ...
            .OnMouseDown((System.Func<GuiMouseEventArgs, ValueTask>)(e => HandleSpinnerMouseDownAsync(e, direction: +1)))
            .OnMouseUp((Action<GuiMouseEventArgs>)(_ => SetSpinnerPressed(up: true, pressed: false)))
            .OnMouseEnter((Action<GuiMouseEventArgs>)(_ => SetSpinnerHovered(up: true, hovered: true)))
            .OnMouseLeave((Action<GuiMouseEventArgs>)(_ => SetSpinnerHovered(up: true, hovered: false)));

        builder.Add<GuiMouseTarget>(2)
            .Configure(target => target.Content = BuildSpinnerDownTargetContent)
            .OnMouseDown((System.Func<GuiMouseEventArgs, ValueTask>)(e => HandleSpinnerMouseDownAsync(e, direction: -1)))
            .OnMouseUp((Action<GuiMouseEventArgs>)(_ => SetSpinnerPressed(up: false, pressed: false)))
            .OnMouseEnter((Action<GuiMouseEventArgs>)(_ => SetSpinnerHovered(up: false, hovered: true)))
            .OnMouseLeave((Action<GuiMouseEventArgs>)(_ => SetSpinnerHovered(up: false, hovered: false)));
    }
    ...
    private async ValueTask HandleSpinnerMouseDownAsync(GuiMouseEventArgs e, int direction)
    {
        if (!Enabled)
        {
            return;
    ...
    protected override ValueTask OnInputMouseDownAsync(GuiMouseEventArgs e)
    {
    ...
    private async ValueTask HandleKeyDownAsync(GuiKeyEventArgs args)
    {
        ...
        if (textChanged && OnTextChanged is GuiCallback<string> onTextChanged)
        {
            await onTextChanged.InvokeAsync(Text);
        }
    }

    private async ValueTask HandleKeyPressAsync(GuiKeyEventArgs args)
    {
        if (!Enabled || !IsFocused)
        {
            return;
