...
public class GuiDropdown<T> : GuiInputBase
{
    ...
    /// <see cref="SetSelectedIndexAsync"/> for an external write that should still raise the
    /// callbacks.</summary>
    ...
    /// or <see cref="SetSelectedIndexAsync"/>.</summary>
    ...
    public async ValueTask SetSelectedIndexAsync(int index)
    {
        ...
        if (OnSelectionChanged is GuiCallback<int> onSelectionChanged)
        {
        ...
        if (clamped >= 0)
        {
            if (OnItemSelected is GuiCallback<T> onItemSelected)
            {
                await onItemSelected.InvokeAsync(Items![clamped]);
            }
        }
    }
    ...
    protected virtual void BuildPopup(IGuiTreeBuilder builder)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            builder.Add<GuiContainer>(i)
                ...
                .OnMouseEnter((Action<GuiMouseEventArgs>)(_ => SetHoveredItem(rowIdx)))
                .OnMouseLeave((Action<GuiMouseEventArgs>)(_ => SetHoveredItem(-1)))
                ...
                .FocusOnMouseDown(this)
                .OnMouseClick((System.Func<GuiMouseEventArgs, ValueTask>)(_ => SelectAndCloseAsync(selIdx)));
        }
    }
        ...
        switch (args.KeyCode)
        {
            ...
            case (int)GlKeys.Up:
                if (Items is { Count: > 0 } itemsUp)
                {
                    ...
                    args.Handled = true;
                    await SetSelectedIndexAsync(prev);
                }
                return;
            ...
            case (int)GlKeys.Down:
                if (Items is { Count: > 0 } itemsDown)
                {
                    ...
                    args.Handled = true;
                    await SetSelectedIndexAsync(next);
                }
                return;
    ...
    /// <inheritdoc/>
    protected override ValueTask OnInputClickAsync(GuiMouseEventArgs e)
    {
    ...
    private void BuildPopupOverlay(IGuiTreeBuilder builder)
    {
        ...
        builder.Add<GuiDropdownPopup>(0)
            .Configure(popup =>
            {
                ...
                popup.Scroll = overflow ? GuiDirection.Vertical : GuiDirection.None;
                popup.Scrollbar = overflow ? GuiDirection.Vertical : GuiDirection.None;
            })
            .ConfigureLayout(layout =>
            {
                layout.Width = GuiLength.Fill;
    ...
    private async ValueTask SelectAndCloseAsync(int actualIndex)
    {
        await SetSelectedIndexAsync(actualIndex);
        Close();
    }
