...
public sealed class GuiSlider : GuiInputBase
{
    ...
    /// <see cref="OnValueChanged"/>; prefer <see cref="SetValueAsync"/> for an external write
    ...
    /// <see cref="Value"/>.
    /// </summary>
    public async ValueTask SetValueAsync(int value)
    {
        ...
        if (OnValueChanged is GuiCallback<int> onValueChanged)
        {
        ...
        GuiBounds? descendantsBounds)
    {
        ...
        double measuredHeight =
            Math.Max(
                0,
                LayoutParameters.Height?.Resolve(null) ?? 24);
    ...
    /// <inheritdoc/>
    protected override async ValueTask OnInputMouseDownAsync(GuiMouseEventArgs e)
    {
        _pressStartValue = Value;
        await UpdateValueFromMouseAsync(e.Position.X);
    }

    /// <inheritdoc/>
    protected override async ValueTask OnInputMouseMoveAsync(GuiMouseEventArgs e)
    {
        if (!IsPressed)
        {
            return;
    ...
    /// <inheritdoc/>
    protected override async ValueTask OnInputMouseUpAsync(GuiMouseEventArgs e)
    {
        if (TriggerOnMouseUp
            && Value != _pressStartValue
            && OnValueChanged is GuiCallback<int> onValueChanged)
        {
            await onValueChanged.InvokeAsync(Value);
        }
    }
    ...
    private async ValueTask UpdateValueFromMouseAsync(double mouseX)
    {
