...
public static class CairoContextExtensions
{
    ...
    public static void RoundRect(this Context ctx, GuiBounds bounds, double radius)
    {
        ...
        ctx.RoundRect(position.X, position.Y, size.Width!.Value, size.Height!.Value, radius);
    }
    ...
    public static void OpenRect(this Context ctx, double x, double y, double w, double h, GuiEdge open)
    {
        ctx.NewPath();
    ...
    /// shadow, etc.).
    /// </summary>
    public static void EdgeLine(this Context ctx, double x, double y, double w, double h, GuiEdge side)
    {
        ctx.NewPath();
        switch (side)
        {
            case GuiEdge.Top: ctx.MoveTo(x, y); ctx.LineTo(x + w, y); break;
            case GuiEdge.Bottom: ctx.MoveTo(x, y + h); ctx.LineTo(x + w, y + h); break;
            case GuiEdge.Left: ctx.MoveTo(x, y); ctx.LineTo(x, y + h); break;
            case GuiEdge.Right: ctx.MoveTo(x + w, y); ctx.LineTo(x + w, y + h); break;
        }
    }
