...
internal class HealthbarTooltip : HudElement
{
    ...
    [
        ...
        ("maximum", "player-health-max"),
        ("percent", "player-health-percent")
    ];

    private readonly string _format;

    public HealthbarTooltip(ICoreClientAPI clientApi, GameStatusService statusService, UiTweaksModConfig.HudConfig.TooltipOptions config)
        : base(clientApi)
    {
        _config = config;
