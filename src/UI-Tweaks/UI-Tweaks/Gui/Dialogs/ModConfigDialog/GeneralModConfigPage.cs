...
internal sealed class GeneralModConfigPage : GuiComponent, IModConfigPage
{
    ...
    protected override void BuildComponentTree(IGuiTreeBuilder builder)
    {
        ...
        BuildSettingRow(builder, key: 1,
            ...
            tooltip: builder => builder.AddLabel(0, Lang.Get($"{Constants.ModId}:config-quicksearch-enable-tooltip")),
        ...
        BuildSettingRow(builder, key: 4,
            label: Lang.Get($"{Constants.ModId}:config-tweaks-meal-remaining-indicator"),
            tooltip: builder => builder.AddLabel(0,
                Lang.Get($"{Constants.ModId}:config-tweaks-meal-remaining-indicator-tooltip")),
            control: builder => builder.AddCheckbox(0,
                checked_: gameTweaks.ShowMealRemainingIndicator,
                onCheckedChanged: value =>
                {
                    gameTweaks.ShowMealRemainingIndicator = value;
                    gameTweaks.NotifyPropertyChanged(nameof(GameTweaksConfig.ShowMealRemainingIndicator));
                    _context!.SaveConfig();
                }));
    }
